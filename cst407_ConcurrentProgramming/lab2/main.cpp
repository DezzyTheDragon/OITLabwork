...
// atomic variables allow multiple threads to safely access and modify the 
// variable in a thread-safe way without using locks. Even though these accesses
// are thread-safe, halgrind will likely complain about them because it doesn't
// know about the semantics of atomic operations. We will cover them later in
// the quarter.
...
static void *Thread_Func(void *arg)
{
    ...
    while (g_running.load())
    {
        usec_delay(args->parallel_delay);
...
int main(int argc, char **argv)
{
    ...
    tsList *list = new tsList(cmd_args.lock_mode);
    
    
    //list.Traverse([](int i){printf("Element is: %d\n", i);});
    
    
    int value;

    unsigned int seed = (unsigned int)usecs();
    printf("Initial list fill\n");
